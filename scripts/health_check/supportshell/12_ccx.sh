#!/usr/bin/env bash
# HC-12: Optional CCX advisory payload ingestion (supportshell path)
set -euo pipefail
...
OUT="${HC_RESULTS_DIR}/${CATEGORY}/ccx_rules.json"
META_OUT="${HC_RESULTS_DIR}/${CATEGORY}/ccx_rules.meta.json"
SCRIPT_PATH="$0"
COMMAND_STR="ingest ccx rules payload"

if [[ -n "${HC_CCX_RULES_FILE:-}" && -f "${HC_CCX_RULES_FILE}" ]]; then
    cp "${HC_CCX_RULES_FILE}" "$OUT"
...
hc_write_capture_metadata "$META_OUT" "$COMMAND_STR" "$SCRIPT_PATH" "$CATEGORY" "ccx_rules"
hc_summary "$CATEGORY"
