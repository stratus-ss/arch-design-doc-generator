...
- `TIER_COUNT` — Number of distinct cluster *types* (not site count). Must be `"1"`, `"2"`, or `"3"`. Count unique types (e.g. DC + Campus + Branch = 3). If every site uses the same cluster type, use `"1"`. If there is no third/edge type, use `"2"`. Do not invent a fourth type. Do not use SITE_COUNT or BRANCH_COUNT here.
- `TIER_PRIMARY` — Name the client uses for the primary/datacenter tier (generic default DC1). Extract "DC", "Datacenter", or similar. Do not use CDF.
- `TIER_MIDDLE` — Name the client uses for the middle tier (generic default DC2). Extract Campus, Regional, Tier 2, Colo. If the ADR renamed CDF, use the current name (e.g. Campus), never CDF. Leave empty when `TIER_COUNT` is 1.
- `TIER_EDGE` — Name the client uses for the third/edge tier (generic default DC3). Extract Branch, Edge, Tier 3 Site, ROBO. Leave empty when `TIER_COUNT` is 1 or 2.
...
### LLD / implementation
Extract these with the same evidence envelope. Do NOT extract shell/snippet variable names (`CLUSTER`, `DOMAIN`, `API_VIP`, `INGRESS_VIP`, `FIO_OUT`, `P99_MS`, `ETCD_DIR`, `INSTALLPLAN`, `INSTALLPLANS`, `SET`, `INFOBLOX_USER`, `INFOBLOX_PASS`).
Keep `IMAGE_REGISTRY` (enterprise registry) distinct from `REGISTRY_MIRROR` / `REGISTRY_MIRROR_FQDN` (disconnected/oc-mirror host). Keep `REGISTRY` distinct from both.
- `GITOPS_HOST` — GitOps git server hostname (host only)
- `CLIENT_DOMAIN` — Client DNS domain used in cluster FQDNs
- `CLIENT_ORG` — Git organization or group name
- `ADR_REGISTER` — ADR register filename or path
- `CVD_URL` — Reference architecture / CVD URL
- `HUB_CLUSTER_NAME` — ACM hub cluster name
- `NTP_DOMAIN` — NTP domain or NTP server domain suffix
- `REGISTRY_MIRROR` — Disconnected/oc-mirror registry host or URL
- `REGISTRY_MIRROR_FQDN` — FQDN of the disconnected/oc-mirror registry
- `REGISTRY` — Container registry referenced in migration procedures
- `SERVER_MODEL` — Server SKU/model if distinct from SERVER_HARDWARE
- `HW_VENDOR` — Hardware vendor word if distinct from SERVER_HARDWARE
- `SITE_LIST` — Comma-separated site names
- `SOURCE_DATASTORE` — VMware source datastore
- `SOURCE_PORT_GROUP` — VMware source port group
- `TARGET_NAD` — Target NetworkAttachmentDefinition
- `TARGET_NAMESPACE` — Target namespace for migrated VMs
- `TARGET_STORAGE_CLASS` — Target StorageClass for migrated VM disks
