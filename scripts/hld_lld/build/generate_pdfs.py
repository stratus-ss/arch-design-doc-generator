...
from config import find_project_yaml, load_config, render_css
...
_PDF_SUBPROCESS_TIMEOUT_SECS = 600


def run(cmd: list[str], *, cwd: Path | None = None, filter_pdf_warnings: bool = False) -> None:
    if not filter_pdf_warnings:
        subprocess.run(cmd, cwd=str(cwd) if cwd else None, check=True, timeout=_PDF_SUBPROCESS_TIMEOUT_SECS)
        return
    ...
    result = subprocess.run(
        ...
        capture_output=True,
        text=True,
        timeout=_PDF_SUBPROCESS_TIMEOUT_SECS,
    )
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="pdfgen-") as tmpdir:
        ...
        run(
            [
                "pandoc",
                # HLD/LLD docs use bare "---" as a visual horizontal rule throughout,
                # not YAML frontmatter; without this, pandoc tries to parse markdown
                # body content between two blank-line-preceded "---" as YAML and crashes.
                "-f",
                "markdown-yaml_metadata_block+autolink_bare_uris",
                str(sub_md),
                "-o",
                str(html),
                "--standalone",
