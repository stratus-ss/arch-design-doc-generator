...
if ! command -v "$STITCHMD" &>/dev/null; then
    ...
    if [[ ! -x "$STITCHMD" ]]; then
        echo "Error: stitchmd not found. Install with: go install go.abhg.dev/stitchmd@v0.9.0" >&2
        exit 1
    fi
fi
...
done < <(python3 "${PROJECT_ROOT}/scripts/shared/lib/config.py" --config "${PROJECT_ROOT}/project.yaml" get-map "hld.summary_map" | python3 -c "import sys,json; [print(k) for k in json.load(sys.stdin).keys()]")
