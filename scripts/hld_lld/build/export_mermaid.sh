...
source "${SCRIPT_DIR}/../../shared/lib/build_helpers.sh"
...
PRIMARY_COLOR_JSON="$(printf '%s' "$PRIMARY_COLOR" | json_str)"
SECONDARY_COLOR_JSON="$(printf '%s' "$SECONDARY_COLOR" | json_str)"
...
cat > "$MMDC_CONFIG" <<JSON
{
  "theme": "default",
  "themeVariables": {
    "primaryColor": ${PRIMARY_COLOR_JSON},
    "primaryTextColor": "#fff",
    "primaryBorderColor": ${SECONDARY_COLOR_JSON},
    "lineColor": ${SECONDARY_COLOR_JSON},
        ...
        if [[ "$in_mermaid" == true ]]; then
            if [[ "$line" == '```' ]]; then
                ...
                if echo "$mermaid_buf" | grep -qE '\{[A-Z_]+\}|\{TBD\}'; then
                    ...
                    echo "  ${outname}"
                    if $MMDC -i "$tmpfile" -o "${outdir}/${outname}" \
                        -s "$SCALE" -c "$MMDC_CONFIG" -b transparent \
                        "${MMDC_PUPPET_ARGS[@]}" \
                        2> >(tee -a "$MMDC_LOG_FILE" >&2); then
                        :
                    else
                        echo "    ⚠ render failed for ${outname}" >&2
...
if [[ $# -ge 1 ]]; then
    ...
    if [[ "$FAIL_COUNT" -gt 0 ]]; then
        echo "Error: ${FAIL_COUNT} mermaid render(s) failed" >&2
        exit 1
    fi
    echo "Done."
    exit 0
