...
- `HC_RECOMMENDATION_AUDIT_LOG.md` is generated locally by `scripts/health_check/update_recommendation_audit_log.py` and is gitignored. The same why-rec argument lives as a `# why-rec` comment above each `recommendation` in `scripts/health_check/hc_report/kb/7_*.toml`. Threshold rationale stays in `HC_CHECK_RATIONALE.md`.
...
- `hc_report/` — report engine:
  ...
  - `kb/` — TOML knowledge base (`7_1`–`7_9` plus `versions.toml`); `[[checks]]` entries may include optional `summary_patterns` for Chapter 4/§6.1 summary matching, plus optional `finding_group`, `finding_group_title`, `include_in_findings` (default true), `finding_on_info` (default false; INFO → P3 finding), and `content_from` (optional exact `check_id` of a canonical row; inherited fields omitted on the alias; single hop; `load_kb` fails closed). Descriptions are mode-neutral (valid without TSR); do not encode a single cluster's TSR remainder. Recommendations list failure classes as examples.
