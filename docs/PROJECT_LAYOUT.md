...
- `HC_RECOMMENDATION_AUDIT_LOG.md` is generated locally by `scripts/health_check/update_recommendation_audit_log.py` and is gitignored. Each generated section shows `recommendation` and `verification` as sibling fields. The same why-rec argument lives as a `# why-rec` comment above each `recommendation` in `scripts/health_check/hc_report/kb/7_*.toml`. Threshold rationale stays in `HC_CHECK_RATIONALE.md`.
