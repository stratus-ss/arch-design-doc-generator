...
- `hc_report/` — report engine:
  ...
  - `kb/` — TOML knowledge base (`7_1`–`7_9` plus `versions.toml`); `[[checks]]` entries may include optional `summary_patterns` for Chapter 4/§6.1 summary matching, plus optional `finding_group`, `finding_group_title`, `include_in_findings` (default true), and `finding_on_info` (default false; INFO → P3 finding). Descriptions are mode-neutral (valid without TSR); do not encode a single cluster's TSR remainder. Recommendations list failure classes as examples.
