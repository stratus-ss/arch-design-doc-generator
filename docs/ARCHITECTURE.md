...
AI is excluded from Health Check **evaluation** by company policy. Optional `HC_SUMMARY_CONCLUSION=1` drafts Chapter 3 and Chapter 8 after generate. Default check profile is `advisory`. HTML and PDF are follow-on container commands (`make hc-html`, `make hc-pdf`) that use `scripts/shared/rendering/hc_export_paths.py` to map each report markdown file to a unique path under `HTML/` or `PDFs/` (preferring `{stem}_pruned.md` when present), then pandoc plus `html_collapsible.py` or `pdf_preprocess.py` and weasyprint; cluster subdirectories are preserved so the same basename in two cluster dirs does not overwrite. Cover-meta colgroups at 42%/58% apply only on that HTML/PDF path.
