...
| **Strength** | A {TIER_PRIMARY} engineer reads only the {TIER_PRIMARY} profile; a {TIER_EDGE} tech reads only the {TIER_EDGE} profile — no filtering needed |
...
This LLD provides a complete Phase 1 (Foundation) specification for each of the three Acme Corp deployment tiers. Each tier profile is self-contained — a reader working on Datacenter deployments can use the {TIER_PRIMARY} profile without referencing {TIER_MIDDLE} or {TIER_EDGE} content.
...
### {TIER_PRIMARY}-2: Hardware & Server Profiles
...
### {TIER_PRIMARY}-3: Network Specification
...
| Parameter | Value |
|---|---|
| NTP servers | {TIER_PRIMARY} internal NTP (SRE-managed) |
...
### {TIER_PRIMARY}-9: Provisioning

Method: ACM ZTP via Assisted Installer. SiteConfig CR applied to {TIER_PRIMARY}/{TIER_MIDDLE} ACM hub.
...
### {TIER_PRIMARY}-10: Pre-Flight Validation
...
### {TIER_PRIMARY}-11: Gate 1 Criteria
...
## {TIER_MIDDLE} Tier Profile

### {TIER_MIDDLE}-1: Tier Overview

| Field | Value |
|---|---|
| **Tier** | {TIER_MIDDLE} / Regional |
| **Sites** | Regional {TIER_MIDDLE} locations |
| **ACM Hub** | {TIER_PRIMARY}/{TIER_MIDDLE} hub |
| **Node topology** | 3 control plane (schedulable) + variable workers |
| **Hardware** | Cisco UCS M8 managed via Intersight |
| **OCP version** | 4.21 (stable channel) |

### {TIER_MIDDLE}-2: Hardware & Server Profiles

Identical to {TIER_PRIMARY} profile (4 vNICs, same Intersight policies, PCI placement enabled). Same Day-0 PSI MachineConfig.

### {TIER_MIDDLE}-3: Network Specification

Same layers as {TIER_PRIMARY}. Key differences:

| Parameter | {TIER_MIDDLE}-Specific |
|---|---|
...
Identical patterns to {TIER_PRIMARY} — site-specific NTP servers, same Infoblox DNS record structure, same certificate issuers.

### {TIER_MIDDLE}-5: Firewall Rules

Same rule set as {TIER_PRIMARY}, including FC SAN ports. Egress: firewall-only.

### {TIER_MIDDLE}-6: Capacity
...
All other capacity parameters match {TIER_PRIMARY}.

### {TIER_MIDDLE}-7: Gate 1 Criteria

Same as {TIER_PRIMARY} Gate 1, with worker count adjusted to site-specific topology.

---

## {TIER_EDGE} Tier Profile

### BR-1: Tier Overview

| Field | Value |
|---|---|
| **Tier** | {TIER_EDGE} (3-node compact on Cisco Unified Edge) |
| **Sites** | ~400 {TIER_EDGE} locations |
| **ACM Hub** | {TIER_EDGE} hub |
...
**Key difference from {TIER_PRIMARY}/{TIER_MIDDLE}:** No dedicated storage or migration vNICs. Traffic combines onto 2 vNICs with bond separation TBD.
...
| Backup | Dedicated | 9000 | {BACKUP_VENDOR} agent backup (WAN to {TIER_PRIMARY}) |
| BMC/CIMC | Site-specific | 1500 | Out-of-band management |

**Not applicable at {TIER_EDGE}:** Storage VLAN, Migration VLAN, FC SAN zoning.
...
**Total: ~11 IPs per {TIER_EDGE} cluster** (vs ~50+ for {TIER_PRIMARY}).

### BR-4: DNS Records

Same record types as {TIER_PRIMARY} (API, API-int, wildcard, per-node A+PTR) — 3 node records instead of 19+.

### BR-5: NTP

| Parameter | Value |
|---|---|
| NTP servers | {TIER_EDGE} network NTP (TBD — unconfirmed) |
| Delivery | chrony MachineConfig via ArgoCD |

**Open item:** {TIER_EDGE} NTP server confirmation required.

### BR-6: Certificates

Same as {TIER_PRIMARY} — API cert from Enterprise CA, Ingress wildcard from Internal CA. Per-cluster wildcard exception (ADR 24).

### BR-7: Firewall Rules

Same inter-node and external rules as {TIER_PRIMARY}. Key differences:

| Difference | Detail |
|---|---|
| Egress model | TBD ({TIER_PRIMARY}/{TIER_MIDDLE} confirmed firewall-only) |
...
No capacity concerns currently anticipated for {TIER_EDGE} workloads.

### BR-9: Provisioning

Method: ACM ZTP via {TIER_EDGE} hub + GitOps ZTP pipeline with SiteConfig CRs for at-scale deployment.
...
Same checklist as {TIER_PRIMARY}, with these differences:

| Check | {TIER_EDGE}-Specific |
|---|---|
...
Parameters that are identical across {TIER_PRIMARY}, {TIER_MIDDLE}, and {TIER_EDGE} — maintained here to avoid duplication.
...
|----|------|------|-------|--------|
| OI-E-01 | {TIER_EDGE} | 2-vNIC layout finalization | Network Team | Open |
| OI-E-02 | {TIER_EDGE} | Egress model decision | Network / Architecture | Open |
| OI-E-03 | {TIER_EDGE} | NTP server confirmation | Network Team | Open |
| OI-E-04 | {TIER_EDGE} | Bandwidth / local Artifactory mirror | Network Team | Open |
