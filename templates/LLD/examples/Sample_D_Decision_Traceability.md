...
**Tier variance:** None — identical across {TIER_PRIMARY}, {TIER_MIDDLE}, and {TIER_EDGE}.
...
**Egress model:** Firewall-only for {TIER_PRIMARY}/{TIER_MIDDLE} (no proxy). {TIER_EDGE} TBD.
...
**Tier variance:** {TIER_EDGE} egress model TBD. FC SAN ports N/A for {TIER_EDGE}.
...
| ID | Item | Owner | Status |
|----|------|-------|--------|
| OI-02-1 | {TIER_EDGE} egress model decision | Network Team / Architecture | Open |
...
| Layer | VLAN | MTU | {TIER_PRIMARY} | {TIER_MIDDLE} | {TIER_EDGE} |
|-------|------|-----|----|----|--------|
...
5. Configure FC SAN zoning ({TIER_PRIMARY}/{TIER_MIDDLE})
...
| ID | Item | Owner | Status |
|----|------|-------|--------|
| OI-03-1 | {TIER_EDGE} 2-vNIC layout finalization | Network Team | Open |
...
| Storage | 1/node | Storage VLAN | Static (NMState) — {TIER_PRIMARY}/{TIER_MIDDLE} only |
| Migration | 1/node | Migration VLAN | Static (NMState) — {TIER_PRIMARY}/{TIER_MIDDLE} only |
...
| ID | Item | Owner | Status |
|----|------|-------|--------|
| OI-05-1 | {TIER_EDGE} NTP server confirmation | Network Team | Open |
...
| Headroom ({TIER_PRIMARY}/{TIER_MIDDLE}) | 1-3 spare nodes | Capacity planning |
| maxUnavailable ({TIER_PRIMARY}) | 2-4 | MachineConfigPool spec |
| maxUnavailable ({TIER_EDGE}) | 1 | MachineConfigPool spec |
...
| ID | Item | Owner | Status |
|----|------|-------|--------|
| OI-10-1 | {TIER_EDGE} bandwidth — local mirror needed? | Network Team | Open |
...
All tiers use ACM ZTP. {TIER_EDGE} clusters additionally use GitOps ZTP pipeline with SiteConfig CRs for at-scale deployment.
...
| OI-02-1 | {TIER_EDGE} egress model decision | Network / Architecture | LLD-02 | Open |
| OI-03-1 | {TIER_EDGE} 2-vNIC layout | Network Team | LLD-03 | Open |
...
| OI-05-1 | {TIER_EDGE} NTP server confirmation | Network Team | LLD-05 | Open |
| OI-10-1 | {TIER_EDGE} bandwidth / local mirror | Network Team | LLD-09 | Open |
