...
| Storage (FC) | WWPN pools, SAN boot targets ({TIER_PRIMARY}/{TIER_MIDDLE} only) |
...
Egress model: Firewall-only for {TIER_PRIMARY}/{TIER_MIDDLE} (ADR 16). {TIER_EDGE} TBD.

### Outputs to Layer 3

- VLANs trunked and reachable between all ports
- Jumbo MTU (9000) confirmed on storage/migration/backup paths
- FC SAN zoning active ({TIER_PRIMARY}/{TIER_MIDDLE})
...
| Storage | 1/node | Storage VLAN | {TIER_PRIMARY}/{TIER_MIDDLE} only |
| Migration | 1/node | Migration VLAN | {TIER_PRIMARY}/{TIER_MIDDLE} only |
...
| Parameter | {TIER_PRIMARY}/{TIER_MIDDLE} | {TIER_EDGE} |
|---|---|---|
| Servers | Internal NTP (SRE-managed) | {TIER_EDGE} network NTP (TBD) |
...
| Tier | Nodes | ACM Hub |
|------|-------|---------|
| {TIER_PRIMARY} | 3 CP (schedulable) + 16+ workers | {TIER_PRIMARY}/{TIER_MIDDLE} hub |
| {TIER_MIDDLE} | 3 CP (schedulable) + variable workers | {TIER_PRIMARY}/{TIER_MIDDLE} hub |
| {TIER_EDGE} | 3 compact | {TIER_EDGE} hub |
...
**Provisioning method:** ACM ZTP via Assisted Installer for all tiers. {TIER_EDGE} additionally uses GitOps ZTP pipeline with SiteConfig CRs.
...
| FC SAN zoning | L2 | No storage path ({TIER_PRIMARY}/{TIER_MIDDLE}) |
