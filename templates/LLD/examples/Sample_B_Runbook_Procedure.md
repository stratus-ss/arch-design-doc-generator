...
6. **Reserve storage interface IPs** (1 per node, {TIER_PRIMARY}/{TIER_MIDDLE} only) on storage VLAN

7. **Reserve migration interface IPs** (1 per node, {TIER_PRIMARY}/{TIER_MIDDLE} only) on migration VLAN
...
1. **Configure VLAN trunking** on Nexus switches for all assigned ports:
   ...
   - Storage VLAN (MTU 9000/9216) — {TIER_PRIMARY}/{TIER_MIDDLE} only
   - Migration VLAN (MTU 9000) — {TIER_PRIMARY}/{TIER_MIDDLE} only
...
- [ ] FC SAN zoning complete ({TIER_PRIMARY}/{TIER_MIDDLE})
