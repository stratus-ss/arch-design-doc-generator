...
| **Applies To** | [{TIER_PRIMARY}] [{TIER_MIDDLE}] [EDGE] |
...
| Parameter | Value | Description | Source |
|-----------|-------|-------------|--------|
| NTP servers | Internal NTP (site-specific) | {TIER_PRIMARY}/{TIER_MIDDLE}: SRE-managed NTP; {TIER_EDGE}: existing network infra | HLD — NTP Requirements |
...
| Parameter | {TIER_PRIMARY} | {TIER_MIDDLE} | {TIER_EDGE} |
|---|---|---|---|
| NTP servers | {TIER_PRIMARY} internal NTP | {TIER_MIDDLE} internal NTP | {TIER_EDGE} network NTP (TBD) |
...
| Parameter | Value | Description | Source |
|-----------|-------|-------------|--------|
| Egress model | Firewall-only (no proxy) | {TIER_PRIMARY}/{TIER_MIDDLE} confirmed; {TIER_EDGE} TBD | ADR 16 |
...
|--------|-----------|-------|--------|
| BIOS | Profile | Cisco "virtualization" preset | CVD baseline (architecture confirmed) |
...
| IPMI | Initial | Encryption disabled (Cisco CVD) | hardware vendor confirmed |
